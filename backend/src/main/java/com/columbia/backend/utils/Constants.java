package com.columbia.backend.utils;

public class Constants {
    public static final String SETTINGS_PATH = "classpath:settings.properties";
    public static final String CITY_ATTRS_PATH = "classpath:cityattrs.properties";
    public static final String NUM_REGEX = "^[+-]?\\d+(\\.\\d+)?$";
    public static final String NAT_INT_REGEX = "^\\d+$";
    public static final String DATE_MD_PATTERN ="MM-dd";
    public static final String DATE_YMD_PATTERN ="yyyy-MM-dd";
    public static final String COVID_HIS_DISPLAY_DAYS_KEY="covid_history_display_days";
    public static final String HOUSING_HIS_DISPLAY_DAYS_KEY="housing_history_display_days";
}
